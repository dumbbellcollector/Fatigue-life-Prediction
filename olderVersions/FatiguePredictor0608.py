# change log: Just Swapped model file to the optuna optimized ones.
...
# TS Classification-based Model and Scaler Paths
MODEL_PATHS = {
    'lowTS': os.path.join(BASE_DIR, 'model_lowTS_optimized.pth'),
    'midTS': os.path.join(BASE_DIR, 'model_midTS_optimized.pth'),
    'highTS': os.path.join(BASE_DIR, 'model_highTS_optimized.pth')
}
SCALER_X_PATHS = {
    'lowTS': os.path.join(BASE_DIR, 'scaler_X_lowTS.pkl'),
    'midTS': os.path.join(BASE_DIR, 'scaler_X_midTS.pkl'),
    'highTS': os.path.join(BASE_DIR, 'scaler_X_highTS.pkl')
}
SCALER_Y_PATHS = {
    'lowTS': os.path.join(BASE_DIR, 'scaler_y_lowTS.pkl'),
    'midTS': os.path.join(BASE_DIR, 'scaler_y_midTS.pkl'),
    'highTS': os.path.join(BASE_DIR, 'scaler_y_highTS.pkl')
}

# --- Optimized Hyperparameters (from Optuna) ---
OPTIMIZED_PARAMS = {
    'lowTS': {
        'n_layers': 3, 'hidden_dim_0': 186, 'hidden_dim_1': 294, 'hidden_dim_2': 64,
        'dropout': 0.2153585570737508,
    },
    'midTS': {
        'n_layers': 6, 'hidden_dim_0': 304, 'hidden_dim_1': 451, 'hidden_dim_2': 275,
        'hidden_dim_3': 421, 'hidden_dim_4': 286, 'hidden_dim_5': 232,
        'dropout': 0.20153194157945667,
    },
    'highTS': {
        'n_layers': 5, 'hidden_dim_0': 727, 'hidden_dim_1': 533, 'hidden_dim_2': 636,
        'hidden_dim_3': 277, 'hidden_dim_4': 751,
        'dropout': 0.4393047612317955,
    }
}
...
# --- TS Classification Function ---
def classify_ts_group(ts_mpa):
    """TS 값에 따라 적절한 그룹 결정"""
    if ts_mpa < 750:
        return 'lowTS'
    elif ts_mpa < 1030:
...
# --- Hybrid Model Prediction and Curve Generation (TS-Classified) ---
def predict_fatigue_curves_hybrid_ts_classified(E_val_gpa, YS_val_mpa, TS_val_mpa, HB_val, 
                                               all_models, all_scaler_X, all_scalers_y, 
                                               target_cols_list, device): 
    
    if any(v is None for v in [E_val_gpa, YS_val_mpa, TS_val_mpa]): # HB_val can be None
        raise ValueError("E, YS, TS 값은 반드시 제공되어야 합니다.")

    # TS 기준 그룹 분류
    ts_group = classify_ts_group(TS_val_mpa)
    
    # 해당 그룹의 모델/스케일러 선택
    model = all_models[ts_group]
    scaler_X = all_scaler_X[ts_group]
    scalers_y_dict = all_scalers_y[ts_group]
    
    #st.info(f"TS = {TS_val_mpa} MPa → {ts_group} 그룹 모델 사용")
...
# --- Load All Group Resources ---
@st.cache_resource
def load_all_group_resources():
    """모든 그룹의 모델과 스케일러를 한 번에 로드 (최적화된 하이퍼파라미터 사용)"""
    input_dim = 4 
    output_dim = 4 
    ...
    for group in ['lowTS', 'midTS', 'highTS']:
        # 파일 존재 확인
        if not os.path.exists(MODEL_PATHS[group]): 
            raise FileNotFoundError(f"Model file not found: {MODEL_PATHS[group]}")
        if not os.path.exists(SCALER_X_PATHS[group]): 
            raise FileNotFoundError(f"Scaler X file not found: {SCALER_X_PATHS[group]}")
        if not os.path.exists(SCALER_Y_PATHS[group]): 
            raise FileNotFoundError(f"Scaler Y file not found: {SCALER_Y_PATHS[group]}")
        
        # 해당 그룹의 최적화된 파라미터 가져오기
        group_params = OPTIMIZED_PARAMS[group]
        hidden_dims = [group_params[f'hidden_dim_{i}'] for i in range(group_params['n_layers'])]
        dropout_p = group_params['dropout']

        # 모델 로드
        model = FatiguePINN(input_dim=input_dim, output_dim=output_dim, 
                           hidden_dims=hidden_dims, dropout_p=dropout_p)
        model.load_state_dict(torch.load(MODEL_PATHS[group], map_location=map_location))
        model.to(_device)
        model.eval()
        all_models[group] = model
        
        # Feature 스케일러 로드
        all_scaler_X[group] = joblib.load(SCALER_X_PATHS[group])
        
        # Target 스케일러 로드
        data_y = joblib.load(SCALER_Y_PATHS[group])
        
        if isinstance(data_y, dict) and 'scalers' in data_y and 'target_cols' in data_y:
            all_scalers_y[group] = data_y['scalers']
            # target_cols는 모든 그룹에서 동일하다고 가정
            if 'target_cols_list' not in locals():
                target_cols_list = data_y['target_cols']
        elif isinstance(data_y, dict):
            all_scalers_y[group] = data_y
            if 'target_cols_list' not in locals():
                target_cols_list = expected_cols_for_model
        else:
            raise ValueError(f"'{SCALER_Y_PATHS[group]}' 파일 형식을 인식할 수 없습니다.")
    
    expected_cols_for_model = ['spf_MPa', 'b', 'epf', 'c'] 
    
    # target_cols_list가 설정되지 않은 경우 기본값 사용
    if 'target_cols_list' not in locals():
        target_cols_list = expected_cols_for_model
        
    # 모든 그룹의 모델 출력 차원 확인
    for group, model in all_models.items():
        if model.output_dim != len(target_cols_list):
            st.error(f"{group} 그룹 모델 출력 차원({model.output_dim})과 타겟 컬럼 수({len(target_cols_list)}) 불일치.")
            raise ValueError(f"{group} 그룹 모델 출력 차원 불일치.")
        
    return all_models, all_scaler_X, all_scalers_y, target_cols_list
...
if 'input_mode' not in st.session_state: st.session_state.input_mode = "정적 물성치 입력"
if 'poisson_ratio_universal' not in st.session_state: st.session_state.poisson_ratio_universal = 0.3 # 공용 포아송 비 초기화

# 모든 그룹 리소스 로드 (앱 시작 시 한 번)
try:
    all_models, all_scaler_X, all_scalers_y, target_cols_list = load_all_group_resources()
    #st.success("모든 TS 그룹별 모델과 스케일러 로드 완료 (lowTS, midTS, highTS)")
except FileNotFoundError as e:
    st.error(f"그룹별 모델 파일 로드 실패: {e}. 앱을 실행할 수 없습니다.")
    st.stop()
...
with st.sidebar:
    ...
    st.radio(
        "입력 방식 선택:",
        ("정적 물성치 입력", "합금 조성비 입력 (wt%)", "물성치 & 조성비 입력"),
        key='input_mode',
        horizontal=False,
        on_change=lambda: setattr(st.session_state, 'prediction_triggered', False)
    )

    if st.session_state.input_mode == "정적 물성치 입력":
        st.subheader("재료 특성")
        ...
        # Optional 합금 조성비 입력 섹션
        with st.expander("합금 조성비 (선택사항, wt%)", expanded=False):
            st.caption("추후 기능 확장을 위한 선택적 입력입니다. 현재는 피로 예측에 사용되지 않습니다.")
            
            col1_comp_opt, col2_comp_opt = st.columns(2)
            element_keys_col1_opt = list(elements_for_input_definition.keys())[:8]
            
            for el_key, props in elements_for_input_definition.items():
                target_col = col1_comp_opt if el_key in element_keys_col1_opt else col2_comp_opt
                with target_col:
                    st.number_input(
                        props['label'], min_value=0.0, value=0.0, 
                        format=props['format'], step=props['step'], key=f"comp_opt_{el_key}",
                        help="선택사항: 추후 기능 확장 시 사용"
                    )
            
            current_composition_sum_opt = sum(st.session_state.get(f"comp_opt_{el}", 0.0) for el in elements_for_input_definition)
            st.caption(f"입력된 조성의 합계: {current_composition_sum_opt:.3f} wt%. (Fe는 잔량으로 간주)")
    ...
    elif st.session_state.input_mode == "물성치 & 조성비 입력":
        st.subheader("재료 특성 & 합금 조성")
        st.info("💡 물성치와 조성비를 모두 입력하여 향상된 예측 성능을 제공합니다. (개발 중)")
        ...
        with col1_prop:
            e_mod_gpa_combined = st.number_input('탄성 계수 (E, GPa)', min_value=1.0, value=200.0, format='%.1f', help="GPa 단위", key="e_mod_gpa_combined")
            ts_mpa_combined = st.number_input('인장 강도 (UTS, MPa)', min_value=1.0, value=700.0, format='%.1f', key="ts_mpa_combined")
            poisson_combined = st.number_input("포아송 비 (ν)", min_value=0.0, max_value=0.5, value=0.3, step=0.01, format='%.2f', help="전단 계산에 사용", key="poisson_combined")
        
        with col2_prop:
            ys_mpa_combined = st.number_input('항복 강도 (YS, MPa)', min_value=1.0, value=500.0, format='%.1f', key="ys_mpa_combined")
            hb_combined = st.number_input('브리넬 경도 (HB)', min_value=0.0, value=200.0, format='%.1f', help="물리 기반 계산 및 모델 입력용", key="hb_combined")
        
        st.markdown("---")
        
        # 조성비 입력 섹션
        st.markdown("##### 합금 조성 (wt%)")
        col1_comp_combined, col2_comp_combined = st.columns(2)
        element_keys_col1_combined = list(elements_for_input_definition.keys())[:8]
        
        for el_key, props in elements_for_input_definition.items():
            target_col = col1_comp_combined if el_key in element_keys_col1_combined else col2_comp_combined
            with target_col:
                st.number_input(
                    props['label'], min_value=0.0, value=props['default'], 
                    format=props['format'], step=props['step'], key=f"comp_combined_{el_key}"
                )
        
        current_composition_sum_combined = sum(st.session_state.get(f"comp_combined_{el}", 0.0) for el in elements_for_input_definition)
        st.caption(f"입력된 조성의 합계: {current_composition_sum_combined:.3f} wt%. (Fe는 잔량으로 간주)")
    ...
    if st.button("피로 거동 예측 실행", use_container_width=True, type="primary"):
        ...
        try:
            if st.session_state.input_mode == "정적 물성치 입력":
                ...
                # 물성치 입력값 가져오기
                e_gpa_to_use = st.session_state.e_mod_gpa_combined
                ys_mpa_to_use = st.session_state.ys_mpa_combined
                ts_mpa_to_use = st.session_state.ts_mpa_combined
                hb_to_use = st.session_state.hb_combined
                nu_to_use = st.session_state.poisson_combined
                
                # 조성비 입력값 가져오기
                current_composition_combined = {el: st.session_state.get(f"comp_combined_{el}", 0.0) for el in elements_for_input_definition}
                
                # 물성치 검증
                if not validate_monotonic_inputs(e_gpa_to_use, ys_mpa_to_use, ts_mpa_to_use, hb_to_use, nu_to_use):
                    raise ValueError("입력된 재료 물성치 유효성 검사 실패.")
                
                # 조성비 검증  
                if not validate_composition_inputs(current_composition_combined):
                    raise ValueError("입력된 합금 조성 유효성 검사 실패.")
                
                st.session_state.user_inputs = {
                    'E_gpa': e_gpa_to_use, 'YS_mpa': ys_mpa_to_use, 'TS_mpa': ts_mpa_to_use,
                    'HB': hb_to_use, 'nu': nu_to_use, 'Input_Mode': input_mode_str,
                    'Composition_Details': current_composition_combined
                }
                
                # 아직 구현되지 않음을 알림
                st.sidebar.warning("⚠️ '물성치 & 조성비 입력' 모드는 현재 개발 중입니다. 추후 업데이트 예정입니다.")
                raise ValueError("물성치 & 조성비 입력 모드는 아직 구현되지 않았습니다.")
            ...
            predicted_en_results = predict_fatigue_curves_hybrid_ts_classified(
                e_gpa_to_use, ys_mpa_to_use, ts_mpa_to_use, hb_for_model,
                all_models, all_scaler_X, all_scalers_y, target_cols_list, device
            )
...
if st.session_state.prediction_triggered and st.session_state.en_results:
    tab_titles = ["피로 파라미터 요약", "인장 곡선 (ϵ-N)", "전단 곡선 (γ-N)", "모델 정보"]
    ...
    with tabs[0]: # Parameter Summary Tab
        ...
        st.markdown("---")
        st.markdown(r"**인장 Coffin-Manson:** $\frac{\Delta\varepsilon}{2} = \frac{\sigma'_f}{E}(2N_f)^b + \varepsilon'_f(2N_f)^c$")
        if shear_p and not np.isnan(shear_p.get('tauf_MPa', np.nan)):
    ...
    with tabs[3]: # Model Info Tab
        st.subheader("모델 정보")
        
        # TS 분류 기준 정보
        st.markdown("#### TS 분류 기준")
        ...
        st.caption("입력된 TS 값에 따라 자동으로 적절한 그룹의 모델이 선택됩니다.")
        
        # 현재 사용된 모델 정보 표시
        if st.session_state.en_results and st.session_state.user_inputs:
            ts_value = st.session_state.user_inputs.get('TS_mpa')
            if ts_value is not None:
                current_group = classify_ts_group(ts_value)
                st.markdown("#### 현재 예측에 사용된 모델")
                st.success(f"**TS = {ts_value} MPa → {current_group} 그룹 모델 사용**")
                
                # 모델 아키텍처 정보
                st.markdown("#### 모델 아키텍처 (최적화된 파라미터)")
                st.write("**FatiguePINN (Physics-Informed Neural Network)**")
                
                # 그룹별 최적 파라미터 동적 표시
                group_params = OPTIMIZED_PARAMS[current_group]
                hidden_dims_display = [group_params[f'hidden_dim_{i}'] for i in range(group_params['n_layers'])]
                dropout_display = group_params['dropout']

                st.write("- **입력 차원:** 4 (E, YS, TS, HB)")
                st.write("- **출력 차원:** 4 (σ'f, b, ε'f, c)")
                st.write(f"- **은닉층:** {hidden_dims_display}")
                st.write(f"- **Dropout:** {dropout_display:.4f}")
                st.write("- **활성화 함수:** ReLU")
                
                # 파일 정보
                st.markdown("#### 로드된 파일")
                st.write(f"**모델 파일:** model_{current_group}.pth")
                st.write(f"**Feature 스케일러:** scaler_X_{current_group}.pkl")
                st.write(f"**Target 스케일러:** scaler_y_{current_group}.pkl")
                
                # 물리 제약 조건
                st.markdown("#### 물리 제약 조건 (Physics-Informed)")
                st.latex(r"\text{Coffin-Manson Relation: } \frac{\Delta\varepsilon}{2} = \frac{\sigma'_f}{E}(2N_f)^b + \varepsilon'_f(2N_f)^c")
                
                # 인장->전단 변환 방법
                st.markdown("#### 인장 → 전단 변환 방법")
                
                col1_conversion, col2_conversion, col3_conversion = st.columns(3)
                
                with col1_conversion:
                    with st.container():
                        st.success("**von Mises 기준**\nTS ≤ 1100 MPa")
                        st.latex(r"\tau'_f = \frac{\sigma'_f}{\sqrt{3}}")
                        st.latex(r"\gamma'_f = \sqrt{3} \cdot \varepsilon'_f")
                
                with col2_conversion:
                    with st.container():
                        st.warning("**선형 보간**\n1100 < TS < 1696 MPa")
                        st.caption("좌측과 우측 방법의 가중평균")
                        st.write("α = (TS-1100)/596")
                        st.write("결과 = (1-α)×좌측 + α×우측")
                
                with col3_conversion:
                    with st.container():
                        st.error("**Maximum Principal**\nTS ≥ 1696 MPa")
                        st.latex(r"\tau'_f = \frac{\sigma'_f}{1 + \nu}")
                        st.latex(r"\gamma'_f = 2 \cdot \varepsilon'_f")
                
                # 현재 사용된 변환 방법 표시
                if st.session_state.shear_results:
                    current_conversion = st.session_state.shear_results.get('conversion_method', 'N/A')
                    st.markdown("#### 현재 예측에 적용된 변환 방법")
                    st.success(f"**{current_conversion}**")
                    
                    if "Interpolated" in current_conversion:
                        st.write("α 값은 TS에 따른 선형 보간 계수입니다:")
                        st.latex(r"\alpha = \frac{TS - 1100}{1696 - 1100}")
                        st.latex(r"\tau'_f = (1-\alpha) \cdot \tau_{vm} + \alpha \cdot \tau_{mp}")
                        st.latex(r"\gamma'_f = (1-\alpha) \cdot \gamma_{vm} + \alpha \cdot \gamma_{mp}")
...
# streamlit run FatiguePredictor0608.py